/**
 * @copyright https://github.com/Aquila169/zod-express-middleware/blob/main/src/index.ts
 * 客製化 error response
 */

import { RequestParamsException } from '@/exceptions/RequestParamsException';
...
export const sendErrors: (errors: Array<ErrorListItem>) => void = (errors) => {
  throw new RequestParamsException(
    errors.map((error) => ({ type: error.type, errors: error.errors.issues })),
  );
};
...
export function processRequestBody<TBody>(
  ...
  return (req, res, next) => {
    ...
    if (parsed.success) {
      ...
      return sendErrors([{ type: 'Body', errors: parsed.error }]);
    }
  };
}
...
export function processRequestParams<TParams>(
  ...
  return (req, res, next) => {
    ...
    if (parsed.success) {
      ...
      return sendErrors([{ type: 'Params', errors: parsed.error }]);
    }
  };
}
...
export function processRequestQuery<TQuery>(
  ...
  return (req, res, next) => {
    ...
    if (parsed.success) {
      ...
      return sendErrors([{ type: 'Query', errors: parsed.error }]);
    }
  };
}
